...
pub(crate)struct EndpointList<E: EndpointTrait + Clone> {
...
impl<E: EndpointTrait + Clone> EndpointList<E> {
    ...
    pub(crate)fn add(&mut self, node: &Node, service: &ServiceItem, data: E::Data) {
        let entry = self
            .endpoints
            .iter_mut()
    ...
    pub(crate)fn next<S: Strategy>(&self, ctx: Option<&Context>, strategy: &RwLock<S>) -> Option<&E> {
    ...
    pub(crate)fn next_local<S: Strategy>(&self, ctx: Option<&Context>, strategy: &RwLock<S>) -> Option<&E> {
    ...
    pub(crate)fn get_endpoint_by_node_id(&self, node_id: &str) -> Option<&E> {
        self.endpoints
            .iter()
