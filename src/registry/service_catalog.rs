...
impl ServiceCatalog {
    ///Add a new service
    pub(crate)fn add(&mut self, node: &Node, service: &ServiceSpec, local: bool) -> String {
    ...
    pub(crate)fn get(&self, full_name: &str, node_id: Option<&str>) -> Option<&ServiceItem> {
        self.services
            .iter()
            .find(|svc| svc.equals(full_name, node_id))
    }
    ...
    pub(crate)fn get_local_node_service(&self) -> Vec<&ServiceItem> {
